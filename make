...
if (file_exists("gateways\\WC_$first_upper" . "_Gateway.php") || strpos($filecontent, "WC_$first_upper" . "_Gateway")) {
...
$myfile = fopen("gateways\\WC_$first_upper" . "_Gateway.php", "w") or die("Unable to open file!");

$txt = '<?php

require_once HYPERPAY_ABSPATH . "includes/Hyperpay_main_class.php";
class WC_' . $first_upper . '_Gateway extends Hyperpay_main_class
{
  /**
   * should be lower case and uniqe
   * @var string $id 
   */
  public $id = "' . strtolower($argv[1]) . '";
  
  /**
   * The title which appear next to gateway on setting page 
   * @var string $method_title
   */
  public $method_title = "' . $first_upper . ' Gateway";
